...
def build_thuy_binh_master():
    ...
    bsdf_skin.inputs['Base Color'].default_value = (0.35, 0.18, 0.09, 1.0) # Warm bronze
    ...
    # Delete generic mannequin head only above chin (keep neck and collarbones intact)
    ...
    head.rotation_euler = (0, 0, math.radians(90)) # Orient face to +X
    ...
    loincloth.parent = rig
    loincloth.parent_type = 'BONE'
    loincloth.parent_bone = 'spine'
    front_flap.parent = rig
    front_flap.parent_type = 'BONE'
    front_flap.parent_bone = 'spine'
    crotch_wrap.parent = rig
    crotch_wrap.parent_type = 'BONE'
    crotch_wrap.parent_bone = 'spine'
    ...
    # 8. Cán Chèo Gỗ Lim Mẫu (Reference Oar & Rowlock)
    bpy.ops.mesh.primitive_cylinder_add(radius=0.0225, depth=2.7, location=(0.28, 0.0, 0.68))
    oar = bpy.context.active_object
    oar.name = 'Can_Cheo_GoLim'
    oar.rotation_euler = (math.radians(10), math.radians(72), math.radians(15))
    oar.data.materials.append(mat_wood)
    for p in oar.data.polygons:
        p.use_smooth = True
        
    bpy.ops.mesh.primitive_cylinder_add(radius=0.020, depth=0.45, location=(0.36, -0.65, 0.68))
    pin = bpy.context.active_object
    pin.name = 'Coc_Cheo_Go'
    pin.data.materials.append(mat_wood)
    
    # 9. Pose Rig & Khóa Khớp Bàn Tay (Kinematics)
    bpy.context.view_layer.objects.active = rig
    ...
    # 9.2 Spine leaning forward 22 deg (Rowing drive phase)
    rot_bone('spine.001', 12, 0, 0)
    rot_bone('spine.002', 8, 0, 0)
    rot_bone('spine.003', 5, 0, 0)
    rot_bone('spine.004', -3, 0, 0)
    rot_bone('spine.005', -4, 0, 0)
    
    # 9.3 Arms reaching forward to grip oar
    ...
    # 9.4 5 Fingers curled tightly around oar cylinder
    ...
    oar.location = mid_hand + Vector((0.015, 0.0, -0.01))
    ...
    cam2.location = (mid_hand.x + 0.55, mid_hand.y - 0.40, mid_hand.z + 0.35)
