...
print("=== [VIETNAM-SIM MASTER PRO] BẮT ĐẦU DỰNG BẠCH ĐẰNG 938 HOÀN HẢO ===")

# 1. DỌN DẸP SẠCH TOÀN BỘ CẢNH CŨ
...
# Các Collection phân lớp chuẩn quốc tế
col_env = bpy.data.collections.new('01_DiaHinh_BoiCanh')
col_coc = bpy.data.collections.new('02_TranDia_BaiCoc')
col_ta = bpy.data.collections.new('03_HamDoi_DaiViet')
col_dich = bpy.data.collections.new('04_HamDoi_NamHan')
...
# -------------------------------------------------------------
# 2. BẦU TRỜI MÙA ĐÔNG 938 & ÁNH SÁNG ĐIỆN ẢNH (WINTER MIST & LOW-ANGLE SUN)
# -------------------------------------------------------------
...
# Sương mù xám lam mùa đông giá rét cửa sông Bạch Đằng (Tháng 12 năm 938)
...
# Ánh mặt trời mùa đông chiếu góc xiên thấp (24 độ so với chân trời)
...
# -------------------------------------------------------------
# 3. BỜ TÂY: DÃY NÚI ĐÁ VÔI KARST TRÀNG KÊNH HÙNG VĨ (A.N.T. LANDSCAPE)
# -------------------------------------------------------------
print("-> Đang sinh Dãy núi Karst Tràng Kênh bằng ANT Landscape chuẩn...")
...
# Đặt tại Bờ Tây, scale Z vút cao 30m sừng sững trên mặt nước
mount_main.location = (-55.0, 0.0, 0.0)
mount_main.scale = (1.0, 1.0, 30.0)

# Vật liệu Đá vôi Karst cổ PBR phong hóa rêu phong
...
mount_sub.location = (-40.0, 120.0, 0.0)
mount_sub.scale = (1.0, 1.0, 25.0)
...
# -------------------------------------------------------------
# 4. BỜ ĐÔNG: BÃI BÙN PHÙ SA & RỪNG SÚ VẸT MAI PHỤC (SAPLING TREE GEN)
# -------------------------------------------------------------
print("-> Đang tạo Bãi bùn phù sa và Rừng sú vẹt bờ Đông...")
bm_mud = bmesh.new()
...
# Sinh cây sú vẹt chân thực bằng Sapling Tree Gen với lá to, xum xuê
...
if bsdf_l:
    bsdf_l.inputs['Base Color'].default_value = (0.04, 0.16, 0.05, 1.0) # Xanh sú vẹt đậm
    bsdf_l.inputs['Roughness'].default_value = 0.40
if base_leaves: base_leaves.data.materials.append(mat_leaf)

# Rải 30 cây sú vẹt tạo thành thảm rừng ngập mặn rậm rạp
random.seed(938)
tree_coords = [
    ...
    (43.0,  80.0), (39.0, -20.0), (40.0,  10.0), (39.0,  40.0), (41.0,  70.0)
]
...
# -------------------------------------------------------------
# 5. MẶT NƯỚC SÔNG BẠCH ĐẰNG & HOẠT CẢNH THỦY TRIỀU RÚT CHUẨN XÁC
# -------------------------------------------------------------
bm_w = bmesh.new()
# Mặt nước rộng 140m x 280m bao trọn lòng sông
...
# -------------------------------------------------------------
# 6. NẠP MASTER ASSETS: THUYỀN CHIẾN & BÃI CỌC BỊT SẮT
# -------------------------------------------------------------
...
# -------------------------------------------------------------
# 7. THIẾT LẬP HIERARCHY & HOẠT CẢNH DIỄN BIẾN 4 PHA (BATTLE PROGRESSION)
# -------------------------------------------------------------
...
# Root Empty cho Thuyền Ta (Ngô Quyền)
...
if main_ta_obj:
    # Reset local transform của main_ta_obj: quay 90 độ quanh Z để mũi thuyền hướng theo trục Y
    main_ta_obj.location = (0, 0, 0)
    main_ta_obj.rotation_euler = (0, 0, math.radians(90))
    main_ta_obj.parent = root_ta

# Root Empty cho Lâu Thuyền Nam Hán (Hoằng Tháo)
...
if main_dich_obj:
    # Reset local transform: quay 90 độ quanh Z để mũi thuyền hướng theo trục Y, mái chèo hai bên
    main_dich_obj.location = (0, 0, 0)
    main_dich_obj.rotation_euler = (0, 0, math.radians(90))
...
if hull_ta:
    ...
    for f in range(1, 301):
        if f < 170:
            pb1.location = (28.0, 15.0, 0.65)
            pb1.rotation_euler = (0, 0, math.radians(180))
            pb2.location = (29.0, -18.0, 0.65)
            pb2.rotation_euler = (0, 0, math.radians(180))
        else:
            p_out = min(1.0, (f - 170) / 75.0)
            pb1.location = (28.0 - p_out * 18.0, 15.0 - p_out * 8.0, -0.45)
            pb1.rotation_euler = (0, 0, math.radians(180 + p_out * 35.0))
            pb2.location = (29.0 - p_out * 19.0, -18.0 + p_out * 14.0, -0.45)
            pb2.rotation_euler = (0, 0, math.radians(180 - p_out * 35.0))
...
for f in range(1, 301):
    if f <= 75:
        # PHA 1: Khiêu chiến và giả thua rút lui ngược dòng lên phía Bắc qua bãi cọc (Y từ -35 đến +14)
        prog = f / 75.0
        ...
        x_pos = 0.5
        z_pos = -0.45
        rot_z = math.radians(180.0) # Giữ hướng Nam tiến công trực diện
...
for f in range(1, 301):
    if f <= 75:
        ...
        # PHA 3: Triều rút cạn, vấp cọc sắt thủng lườn! Khựng lại, nghiêng 26 độ sang mạn phải!
        hit_prog = (f - 125) / 55.0
        smooth_hit = hit_prog * hit_prog * (3 - 2 * hit_prog)
        y_pos = 0.0 + smooth_hit * 1.5
        x_pos = smooth_hit * 0.6
        z_pos = 0.15 - smooth_hit * 0.55
        # Nghiêng mạn (Roll quanh trục Y), chúi mũi (Pitch quanh trục X), trôi lệch hướng (Yaw quanh Z)
        rot_x = math.radians(-5.0 * smooth_hit)
        rot_y = math.radians(26.0 * smooth_hit)
        rot_z = math.radians(15.0 * smooth_hit)
    else:
        ...
        rot_x = math.radians(-5.0 + math.sin(f * 0.2) * 0.5)
        rot_y = math.radians(26.0 + math.sin(f * 0.25) * 0.6)
        rot_z = math.radians(15.0)
...
# -------------------------------------------------------------
# 8. CAMERA ĐIỆN ẢNH & GÓC NHÌN VIEWPORT TOÀN CẢNH
# -------------------------------------------------------------
...
# Đặt góc máy điện ảnh nhìn từ trên cao phía Đông Nam bao quát toàn cảnh
cam_obj.location = (26.0, -42.0, 18.0)
cam_obj.rotation_euler = (math.radians(68), 0, math.radians(34))
col_light.objects.link(cam_obj)
scene.camera = cam_obj

# Cập nhật Viewport 3D sang chế độ MATERIAL Shading
for window in bpy.context.window_manager.windows:
    for area in window.screen.areas:
        if area.type == 'VIEW_3D':
            ...
            r3d.view_location = (-4.0, -8.0, 4.0)
            r3d.view_rotation = Euler((math.radians(64), 0.0, math.radians(32)), 'XYZ').to_quaternion()
...
print("=== [VIETNAM-SIM MASTER PRO] HOÀN TẤT 100%! BỐI CẢNH SIÊU THỰC & CHUẨN LỊCH SỬ! ===")
