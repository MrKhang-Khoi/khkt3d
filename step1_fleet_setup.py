...
print("=== [VIETNAM-SIM] BƯỚC 1: BỐI CẢNH TRIỀU DÂNG & HẠM ĐỘI HAI BÊN ===")

# 1. DỌN DẸP SẠCH CẢNH CŨ
...
scene.frame_start = 1
scene.frame_end = 80  # Bước 1 tập trung chuẩn xác từ Frame 1 đến Frame 80
scene.render.fps = 24
scene.frame_current = 1

# Tạo các Collection phân lớp chuyên nghiệp cho Bước 1
...
# -------------------------------------------------------------
# 2. BẦU TRỜI MÙA ĐÔNG 938 & ÁNH SÁNG ĐIỆN ẢNH (WINTER MIST & LOW-SUN)
# -------------------------------------------------------------
...
# Sương mù xám lam mùa đông giá rét (Tháng 12 năm 938)
...
# Mặt trời mùa đông chiếu xiên góc thấp 24 độ
...
print("-> Bầu trời và ánh sáng mùa đông hoàn tất")

# -------------------------------------------------------------
# 3. BỜ TÂY: DÃY NÚI ĐÁ VÔI KARST TRÀNG KÊNH (A.N.T. LANDSCAPE)
# -------------------------------------------------------------
print("-> Đang kiến tạo Dãy núi Karst Tràng Kênh...")
...
# -------------------------------------------------------------
# 4. BỜ ĐÔNG: BÃI BÙN PHÙ SA & RỪNG SÚ VẸT NGẬP MẶN (SAPLING TREE GEN)
# -------------------------------------------------------------
...
# Sinh cây sú vẹt chân thực bằng Sapling Tree Gen
...
# Rải 28 cây sú vẹt tạo thành thảm rừng ngập mặn rậm rạp
random.seed(938)
tree_coords = [
...
# -------------------------------------------------------------
# 5. MẶT NƯỚC SÔNG BẠCH ĐẰNG Ở BƯỚC 1: TRIỀU CƯỜNG DÂNG CAO (+0.85m)
# -------------------------------------------------------------
bm_w = bmesh.new()
...
# Ở Bước 1: Triều dâng cao vững chắc ở mức Z = +0.85m (ngập sâu bãi cọc 1m)
anim_w = obj_water.animation_data_create()
act_w = bpy.data.actions.new(name='Action_ThuyTrieu_B1')
anim_w.action = act_w
for f in range(1, 81):
    zt = 0.85 + math.sin(f * 0.08) * 0.015 # Dao động sóng triều nhẹ
    obj_water.location = (0, 0, zt)
    obj_water.keyframe_insert(data_path='location', frame=f)

print("-> Mặt nước triều dâng (+0.85m) đã thiết lập hoàn tất")

# -------------------------------------------------------------
# 6. BÃI CỌC BẠCH ĐẰNG NGẬP SÂU DƯỚI ĐÁY SÔNG
# -------------------------------------------------------------
assembly_path = r'C:\Users\HPZBook\Desktop\TEST_BLENDER\scenes\dai_chien_bach_dang_assembly.blend'
print(f"-> Nạp 25 cọc Bạch Đằng từ {assembly_path}...")
    ...
    if c_imp:
        for ob in c_imp.objects:
            if 'Coc' in ob.name and ob.name not in col_coc.objects:
                # Đảm bảo cọc cắm sâu ở đáy sông (đầu cọc ở Z = -0.15m đến -0.05m)
                # Ngập sâu dưới mặt nước (+0.85m) ít nhất 0.9m - 1.0m!
                col_coc.objects.link(ob)

print("-> 25 cọc Bạch Đằng ngập sâu vô hình dưới dòng nước triều cường!")

# -------------------------------------------------------------
# 7. XÂY DỰNG ĐỘI HÌNH HẠM ĐỘI HAI BÊN CHUẨN XÁC CHÍNH SỬ
# -------------------------------------------------------------
# Nạp Master Ships từ blend files
with bpy.data.libraries.load(assembly_path, link=False) as (data_from, data_to):
...
# Hàm nhân bản hạm đội nguyên vẹn cây phân cấp (Hierarchy Cloning)
...
# --- A. HẠM ĐỘI TIÊN PHONG ĐẠI VIỆT (3 CHIẾN THUYỀN NHẸ BUỒM CÁNH DƠI) ---
# Thuyền 1: Thuyền Soái Tiên Phong (Ngô Quyền / Tướng chỉ huy)
# Thuyền 2: Thuyền Tiên Phong Tả Dực (Sườn Tây)
# Thuyền 3: Thuyền Tiên Phong Hữu Dực (Sườn Đông)

# Chuẩn hóa master_ta_root: quay 90 độ quanh Z để mũi thuyền hướng theo trục Y
...
# Tạo Root điều khiển cho 3 thuyền Đại Việt để đảm bảo CÂN BẰNG TUYỆT ĐỐI (Roll = 0°)
viet_fleet = [
    {"name": "Root_DaiViet_1_Soai", "obj": master_ta_root, "offset_x": 0.0, "offset_y": 8.0, "scale": 1.0},
    {"name": "Root_DaiViet_2_TaDuc", "obj": ta_ship_2, "offset_x": -7.5, "offset_y": 18.0, "scale": 0.9},
    {"name": "Root_DaiViet_3_HuuDuc", "obj": ta_ship_3, "offset_x": +8.0, "offset_y": 15.0, "scale": 0.9}
]

for item in viet_fleet:
    root_e = bpy.data.objects.new(item["name"], None)
    root_e.empty_display_type = 'ARROWS'
    root_e.empty_display_size = 2.5
    ...
    # Keyframe chuyển động Bước 1 (Frames 1-80): Rút lui ngược dòng lên phía Bắc (+Y)
    ...
    for f in range(1, 81):
        prog = (f - 1) / 79.0
        cur_y = start_y + prog * 38.0  # Rút lui tốc độ cao theo trục Y
        cur_x = start_x + math.sin(prog * 2.5) * 1.2 # Lướt sóng nhẹ
        # Mớn nước nổi chuẩn xác trên mặt nước triều dâng (+0.85m)
        cur_z = 0.85 + math.sin((f + start_x) * 0.12) * 0.02
        
        # KHÓA CHẶT ROLL = 0.0 ĐỂ KHÔNG BỊ NGHIÊNG MẠN!
        # Chỉ có pitch dập dềnh nhẹ theo nhịp chèo và sóng
        pitch = math.sin(f * 0.2) * math.radians(0.6)
        roll = 0.0  # TUYỆT ĐỐI CÂN BẰNG
...
print("-> Đội thuyền Tiên phong Đại Việt (3 thuyền nhẹ) hoàn tất thăng bằng 100%!")

# --- B. HẠM ĐỘI VIỄN CHINH NAM HÁN (1 ĐẠI LÂU THUYỀN + 2 CHIẾN THUYỀN HỘ VỆ) ---
# Chuẩn hóa master_dich_root
...
# Tạo 2 thuyền hộ vệ Nam Hán (chiến thuyền tầm trung hai bên sườn)
han_escort_1 = clone_ship_hierarchy(master_dich_root, "NamHan_HoVe_Ta", col_dich)
han_escort_2 = clone_ship_hierarchy(master_dich_root, "NamHan_HoVe_Huu", col_dich)

han_fleet = [
    {"name": "Root_NamHan_1_SoaiHam_LauThuyen", "obj": master_dich_root, "offset_x": 0.0, "offset_y": -46.0, "scale": 1.0},
    {"name": "Root_NamHan_2_HoVe_Tay", "obj": han_escort_1, "offset_x": -15.0, "offset_y": -34.0, "scale": 0.65},
    {"name": "Root_NamHan_3_HoVe_Dong", "obj": han_escort_2, "offset_x": +15.0, "offset_y": -37.0, "scale": 0.65}
]

for item in han_fleet:
    root_e = bpy.data.objects.new(item["name"], None)
    root_e.empty_display_type = 'ARROWS'
    root_e.empty_display_size = 4.0
    ...
    for f in range(1, 81):
        prog = (f - 1) / 79.0
        cur_y = start_y + prog * 45.0  # Hăm hở truy đuổi rút ngắn cự ly
        cur_x = start_x
        # Mớn nước sâu đầm chắc trên mặt nước triều cường
        cur_z = 0.85 + math.sin((f + start_x) * 0.08) * 0.015
        
        # KHÓA CHẶT ROLL = 0.0 (KHÔNG NGHIÊNG, ĐẦM CHẮC VỮNG CHÃI)
        pitch = math.sin(f * 0.12) * math.radians(0.4)
        roll = 0.0  # TUYỆT ĐỐI CÂN BẰNG
        yaw = 0.0   # TIẾN THẲNG HƯỚNG BẮC (+Y) TRUY ĐUỔI
...
print("-> Hạm đội Nam Hán (1 Soái hạm + 2 Hộ vệ thuyền) hoàn tất thăng bằng 100%!")

# -------------------------------------------------------------
# 8. CAMERA ĐIỆN ẢNH & THIẾT LẬP VIEWPORT QUAN SÁT BƯỚC 1
# -------------------------------------------------------------
cam_data = bpy.data.cameras.new("Cam_B1_ChienTruong")
cam_data.lens = 35.0
cam_data.clip_end = 800.0
cam_obj = bpy.data.objects.new("Camera_B1_QuanSat", cam_data)
# Đặt góc máy cao bao quát: Núi Karst bờ Tây bên trái, Rừng sú vẹt bên phải,
# Đội thuyền Đại Việt phía trước, Hạm đội Nam Hán phía sau đang truy kích
cam_obj.location = (28.0, -48.0, 24.0)
cam_obj.rotation_euler = (math.radians(65), 0, math.radians(32))
col_light.objects.link(cam_obj)
scene.camera = cam_obj

# Cập nhật Viewport 3D sang Material Shading
for window in bpy.context.window_manager.windows:
    for area in window.screen.areas:
        if area.type == 'VIEW_3D':
            ...
            r3d.view_distance = 85.0
            r3d.view_location = (0.0, -10.0, 5.0)
            r3d.view_rotation = Euler((math.radians(64), 0.0, math.radians(30)), 'XYZ').to_quaternion()
...
print("=== [VIETNAM-SIM] HOÀN TẤT THIẾT LẬP BƯỚC 1 CHUẨN XÁC LỊCH SỬ 100%! ===")
