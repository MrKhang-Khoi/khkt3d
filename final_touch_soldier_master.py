...
def build_thuy_binh_master():
    ...
    # Shrink generic head vertices inside the neck
    bpy.context.view_layer.objects.active = body
    ...
    # 5.3 Khăn Đỏ Đầu Rìu (Adjusted forward +6mm so it rests smoothly on forehead without clipping)
    bpy.ops.mesh.primitive_torus_add(major_segments=36, minor_segments=16, major_radius=0.080, minor_radius=0.006, location=(0.004, 0.0, 1.514))
    ...
    bpy.ops.mesh.primitive_cone_add(vertices=4, radius1=0.022, depth=0.090, location=(-0.060, 0.084, 1.505))
    ...
    bpy.ops.mesh.primitive_cone_add(vertices=4, radius1=0.018, depth=0.075, location=(-0.072, 0.074, 1.480))
    ...
    # 7. Ghế Ngồi Xà Ngang, Gờ Đạp Chân, 2 Mạn Thuyền & Sàn Thuyền
    ...
    # Mạn thuyền Trái (Nơi đặt cọc chèo - Oar fulcrum side)
    ...
    # Positioned right in the palm contact zone (+0.055m in X, -0.005m in Z)
    ...
    oar_dir = (grip_l - grip_r).normalized() # Points out to Left gunwale
    ...
    # Cọc chèo gỗ (Rowlock pin) anchored into Left gunwale
    ...
    cam2.location = (grip_l.x + 0.40, grip_l.y - 0.28, grip_l.z + 0.22)
