...
def build_thuy_binh_master():
    ...
    # 2.5 Cán Chèo Gỗ Lim & Mạn Thuyền
    ...
    # Shrink generic head vertices inside the neck rather than deleting (prevents open hole)
    bpy.context.view_layer.objects.active = body
    ...
    # Position head on neck: align with collarbones
    ...
    # 5. Phục Sức Đầu: Búi Tóc Củ Tỏi & Khăn Đỏ Đầu Rìu
    # 5.1 Tóc nền (Snug hair cap fitting scalp closely)
    ...
    hair_cap.scale = (0.88, 0.82, 0.72) # snug fit
    bpy.ops.object.transform_apply(scale=True)
    ...
    bpy.ops.mesh.primitive_uv_sphere_add(segments=32, ring_count=24, radius=0.035, location=(-0.080, 0.0, 1.530))
    topknot = bpy.context.active_object
    topknot.name = 'Bui_Toc_CuToi'
    topknot.scale = (0.85, 0.85, 1.05)
    bpy.ops.object.transform_apply(scale=True)
    ...
    bpy.ops.mesh.primitive_torus_add(major_radius=0.028, minor_radius=0.004, location=(-0.072, 0.0, 1.508))
    ...
    # 5.3 Khăn Đỏ Đầu Rìu (Flat ribbon across forehead)
    bpy.ops.mesh.primitive_torus_add(major_segments=36, minor_segments=16, major_radius=0.078, minor_radius=0.005, location=(-0.005, 0.0, 1.512))
    headband = bpy.context.active_object
    headband.name = 'Khan_DauRiu_Vong'
    headband.rotation_euler = (0, math.radians(-8), 0)
    headband.scale = (1.02, 0.86, 1.3)
    bpy.ops.object.transform_apply(rotation=True, scale=True)
    ...
    bpy.ops.mesh.primitive_cone_add(vertices=4, radius1=0.022, depth=0.090, location=(-0.062, 0.082, 1.505))
    ...
    bpy.ops.mesh.primitive_cone_add(vertices=4, radius1=0.018, depth=0.075, location=(-0.074, 0.072, 1.480))
    ...
    # 7. Ghế Ngồi Xà Ngang, Gờ Đạp Chân, Mạn Thuyền & Sàn Thuyền
    ...
    # Mạn thuyền (Gunwale) dọc bên phải
    bpy.ops.mesh.primitive_cube_add(size=1.0, location=(0.35, -0.68, 0.50))
    gunwale = bpy.context.active_object
    gunwale.name = 'Man_Thuyen_BeGo'
    gunwale.scale = (1.6, 0.10, 0.30)
    bpy.ops.object.transform_apply(scale=True)
    gunwale.data.materials.append(mat_wood)
    ...
    # 9. Cán Chèo Gỗ Lim Mẫu (Reference Oar & Rowlock)
    # Align cylinder directly across the palm grasping zone
    hand_l = rig.matrix_world @ rig.pose.bones['hand.L'].head
    hand_r = rig.matrix_world @ rig.pose.bones['hand.R'].head
    
    # Palm contact surface is offset by +0.035m in X and -0.015m in Z
    palm_offset = Vector((0.035, 0.0, -0.015))
    grip_l = hand_l + palm_offset
    grip_r = hand_r + palm_offset
    
    bpy.ops.mesh.primitive_cylinder_add(radius=0.0225, depth=2.8)
    ...
    oar_dir = (grip_r - grip_l).normalized()
    handle_end = grip_l - oar_dir * 0.45
    oar_center = handle_end + oar_dir * 1.40
    ...
    # Cọc chèo gỗ (Rowlock pin) firmly fixed to gunwale
    pin_loc = handle_end + oar_dir * 1.62
    bpy.ops.mesh.primitive_cylinder_add(radius=0.022, depth=0.45, location=(pin_loc.x, pin_loc.y, pin_loc.z - 0.05))
    ...
    target_hand.location = grip_l
    
    target_body = bpy.data.objects.new('Target_Body', None)
    bpy.context.scene.collection.objects.link(target_body)
    target_body.location = (0.15, 0.0, 0.82) # Center of seated rower
    ...
    cam1.location = (head_center.x + 0.68, head_center.y - 0.26, head_center.z + 0.03)
    tt1 = cam1.constraints.new('TRACK_TO')
    tt1.target = target_head
    tt1.track_axis = 'TRACK_NEGATIVE_Z'
    tt1.up_axis = 'UP_Y'
    
    # Cam 2: Hand Grip Macro Detail (Looking tightly at Left Hand wrapping around oar)
